...
# ----------------------- Wrapper extra functionality ----------------------- #
    def _process_metric(self, metric, metric_name) -> Union[np.ndarray, None]:
        ...
        if metric.shape[0] != self.observation_space.shape[0]:
            self.logger.error(
                f"{metric_name} shape mismatch: expected {self.observation_space.shape[0]}, got {metric.shape[0]}"
            )
            raise ValueError
...
class EnergyCostWrapper(gym.Wrapper):
    ...
    def __init__(self,
        ...
        self.recalculate_reward = bool(recalculate_reward)
        reward_kwargs = dict(reward_kwargs or {})
        if self.recalculate_reward:
            available_observations = set(self.observation_variables)
            checked_keys = (
                'temperature_variables',
                'energy_variables',
                'energy_cost_variables',
            ...
            missing_variables = [
                ...
                for variable_name in reward_kwargs.get(key, [])
                if variable_name not in available_observations
            ]
            if missing_variables:
                self.logger.warning(
                    'EnergyCostWrapper reward recalculation disabled because '
                    f'variables are not in the observation space: {missing_variables}'
                )
                self.recalculate_reward = False
        ...
        self.reward_fn = EnergyCostLinearReward(**reward_kwargs) if self.recalculate_reward else None
        self.logger.info('Wrapper initialized.')
    ...
    def step(self, action: np.ndarray
        ...
        if not self.recalculate_reward or self.reward_fn is None:
        ...
        try:
            ...
            self.logger.warning(
                'EnergyCostWrapper reward recalculation disabled after '
                f'missing observation variable: {exc}'
            )
            self.recalculate_reward = False
            return new_obs, reward, terminated, truncated, info
        info = {
    ...
    class WandBLogger(gym.Wrapper):  # pragma: no cover
        ...
        def __init__(self,
            ...
            run_name = run_name or (
                f"{self.env.get_wrapper_attr('name')}_{wandb.util.generate_id()}"
            )
        ...
        def step(self, action: np.ndarray
            ...
            if self.global_timestep % self.dump_frequency == 0:
                self.logger.debug(
                    f"Dump frequency reached ({self.global_timestep}), logging to WandB."
                )
                self.wandb_log()
        ...
        def reset(self,
            ...
            if self.get_wrapper_attr('is_running'):
                ...
                if self.get_wrapper_attr(
                    ...
                    self.logger.warning(
                        f"Episode ignored for log summary in WandB Platform, it has not be completed in at least {self.episode_percentage * 100}%."
                    )
                self.logger.info(
...
class VariabilityContextWrapper(gym.Wrapper):
    ...
    def step(self, action: np.ndarray
        ...
        if self.next_step_update == 0:
            ...
            self.logger.info(
                f"Context updated with values: {self.next_context_values}"
            )
