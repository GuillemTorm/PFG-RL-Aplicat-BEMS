...
def DEFAULT_SHOP_DISCRETE_FUNCTION(action: int) -> np.ndarray:
    ...
    thermostat_action = action % len(thermostat_mapping)
    battery_action = action // len(thermostat_mapping)

    return np.array(
        [
            *thermostat_mapping[thermostat_action],
            *battery_mapping[battery_action]
        ],
        dtype=np.float32)
